...
class Env(gym.Env, ABC):
    ...
    def __init__(self, staged=False, action_space=7, recognize=False):
    ...
    def load_models(self):
        self.detector = YOLOv5DetectorLearner(model_name='yolov5x', device='cpu')
        # self.detector = SingleShotDetectorLearner(device='cuda')
        # self.detector.download(".", mode="pretrained")
        # self.detector.load("./ssd_default_person", verbose=True)
    ...
    def step(self, action):
        ...
        ## Below method has cv2.waitKey(0) and awaits for a keyboard input.
        # boxes = self.detector.infer(obs)
        # draw_bounding_boxes(obs, boxes, class_names=self.detector.classes, show=True)
        # alternatively use detect method
        obs = self.detect(obs)
